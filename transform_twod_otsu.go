...
// TwoDOtsu implements MATHEMATICALLY CORRECT 2D Otsu thresholding with guided filtering
...
func (t *TwoDOtsu) applyWithScale(src gocv.Mat, scale float64) gocv.Mat {
	...
	t.debugImage.LogAlgorithmStep("2D Otsu Fixed", fmt.Sprintf("Starting binarization (scale: %.2f)", scale))

	if src.Empty() {
		t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "ERROR: Input matrix is empty")
		return gocv.NewMat()
	}
	...
	if scale != 1.0 {
		...
		if newWidth <= 0 || newHeight <= 0 || newWidth > 16384 || newHeight > 16384 {
			t.debugImage.LogAlgorithmStep("2D Otsu Fixed", fmt.Sprintf("ERROR: Invalid scaled dimensions: %dx%d", newWidth, newHeight))
			return gocv.NewMat()
		}
		...
		t.debugImage.LogAlgorithmStep("2D Otsu Fixed", fmt.Sprintf("Scaled to %dx%d", newWidth, newHeight))
	} else {
	...
	if grayscale.Empty() {
		t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "ERROR: Grayscale conversion failed")
		return gocv.NewMat()
	}
	...
	if guided.Empty() {
		t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "ERROR: Guided filter failed")
		guided = grayscale.Clone()
		defer guided.Close()
	}

	// Apply CORRECTED 2D Otsu thresholding
	...
	if binaryResult.Empty() {
		t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "ERROR: Binarization failed")
		return gocv.NewMat()
	}
	...
	if scale != 1.0 {
		...
		processed.Close()
		t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "Scaled back to original size")
	} else {
		result = processed
	}

	t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "Completed successfully")
	return result
}
...
func (t *TwoDOtsu) applyGuidedFilterFixed(src gocv.Mat, windowRadius int, epsilon float64) gocv.Mat {
	...
	t.debugImage.LogFilter("GuidedFilter Fixed", fmt.Sprintf("radius=%d epsilon=%.3f", windowRadius, epsilon))
	return result
}

// FIXED: Mathematically correct 2D Otsu algorithm implementation
func (t *TwoDOtsu) apply2DOtsuFixed(gray, guided gocv.Mat) gocv.Mat {
	t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "Constructing 2D histogram with correct algorithm")
	...
	if len(grayData) == 0 || len(guidedData) == 0 || len(grayData) != len(guidedData) {
		t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "ERROR: Invalid data")
		return gocv.NewMat()
	}
	...
	// CORRECTED: Find optimal thresholds using proper 2D Otsu formulation
	...
	// Process pixel by pixel using CORRECT 2D Otsu classification
	for y := 0; y < height; y++ {
		for x := 0; x < width; x++ {
	...
	t.debugImage.LogAlgorithmStep("2D Otsu Fixed", "Binarization completed with corrected algorithm")
	return result
}

// FIXED: Correct 2D Otsu optimal threshold finding using between-class scatter matrix
func (t *TwoDOtsu) findOptimalThresholdsFixed(hist [256][256]float64) (int, int, float64) {
	maxVariance := 0.0
	bestS, bestT := 0, 0
	...
	for s := 1; s < 255; s++ {
		for thresholdT := 1; thresholdT < 255; thresholdT++ {
			// CORRECTED: Calculate proper between-class scatter matrix trace
...
// FIXED: Proper 2D between-class scatter matrix trace calculation
func (t *TwoDOtsu) calculateBetweenClassScatterTrace(hist [256][256]float64, s, thresholdT int) float64 {
...
// FIXED: Statistical classification based on proper 2D Otsu theory
func (t *TwoDOtsu) classifyPixelStatistically(grayVal, guidedVal, bestS, bestT int, hist [256][256]float64) bool {
...
func (t *TwoDOtsu) createParameterUI() *fyne.Container {
	...
	radiusEntry.OnSubmitted = func(text string) {
		if value, err := strconv.Atoi(text); err == nil && value >= 1 && value <= 20 {
			...
			t.debugImage.LogAlgorithmStep("2D Otsu Fixed Parameters", fmt.Sprintf("Window radius changed: %d -> %d", oldValue, value))
			if t.onParameterChanged != nil {
				go t.onParameterChanged()
			}
		} else {
			t.debugImage.LogAlgorithmStep("2D Otsu Fixed Parameters", fmt.Sprintf("Invalid window radius: %s (must be 1-20)", text))
		}
	}
	...
	epsilonEntry.OnSubmitted = func(text string) {
		if value, err := strconv.ParseFloat(text, 64); err == nil && value > 0.001 && value <= 1.0 {
			...
			t.debugImage.LogAlgorithmStep("2D Otsu Fixed Parameters", fmt.Sprintf("Epsilon changed: %.3f -> %.3f", oldValue, value))
			if t.onParameterChanged != nil {
				go t.onParameterChanged()
			}
		} else {
			t.debugImage.LogAlgorithmStep("2D Otsu Fixed Parameters", fmt.Sprintf("Invalid epsilon: %s (must be 0.001-1.0)", text))
		}
	}
	...
	kernelEntry.OnSubmitted = func(text string) {
		if value, err := strconv.Atoi(text); err == nil && value >= 1 && value <= 15 && value%2 == 1 {
			...
			t.debugImage.LogAlgorithmStep("2D Otsu Fixed Parameters", fmt.Sprintf("Morphological kernel size changed: %d -> %d", oldValue, value))
			if t.onParameterChanged != nil {
				go t.onParameterChanged()
			}
		} else {
			t.debugImage.LogAlgorithmStep("2D Otsu Fixed Parameters", fmt.Sprintf("Invalid kernel size: %s (must be 1-15 and odd)", text))
		}
	}
