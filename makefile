# Image Restoration Suite Makefile - FIXED VERSION
...
# Default target - production build without profiling
build:
	@echo "Building production binary..."
	go build -ldflags="-s -w" -o $(BINARY_NAME) .

# FIXED: Enhanced profiling build with comprehensive memory tracking
build-profile:
	@echo "Building with Mat profiling and enhanced memory tracking..."
	go build -tags matprofile -ldflags="-s -w" -o $(BINARY_NAME) .

# Production run without profiling
run:
	@echo "Running production build..."
	go run .

# FIXED: Enhanced profiling run with memory leak detection
run-profile:
...
# FIXED: Enhanced memory leak detection
check-leaks:
...
# FIXED: Development workflow target
...
help:
	...
	@echo ""
	@echo "Quick start:"
	@echo "  make dev && make run-profile"